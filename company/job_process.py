    ...
    try:
        ...
        for id, job_info, category_name, job_name in zhilian_job:
            try:
                ...
                if 'cardCustomJson' in filtered_data and filtered_data['cardCustomJson']:
                    try:
                        card_custom_data = json.loads(filtered_data['cardCustomJson'])
                        # 提取salary60到最外层
...
def process_job_batch(cursor, connection, batch_size=20000):
    """处理一批岗位数据"""
    cursor.execute("SELECT id, job_info, category_name, job_name FROM zhilian_job LIMIT %s",
                  (batch_size,))
