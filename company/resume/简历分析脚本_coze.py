...
counter = 0
bot_id_index = 0  # 用于轮询bot_ids的索引
bot_id_lock = threading.Lock()  # 用于保护bot_id_index的线程锁
...
def process_chunk(chunk, threadName, lock, output_dir):
    ...
    column_names.extend(['序号','introduce', 'threadName', 'startTime', 'endTime', 'startTime_format', 'endTime_format', '耗时(秒)', 'bot_id'])
    ...
    for index, row in chunk.iterrows():
        try:
            ...
            error_row.extend([counter, f"处理出错: {str(e)}", threading.current_thread().name, 0, 0, 0, 0, 0, "未知"])
...
def api_call(row, AppConversationID, lock, user_id, counter):
    global bot_id_index, bot_id_lock
    # global counter
    ...
    # 轮询获取bot_id
    with bot_id_lock:
        current_bot_id = bot_ids[bot_id_index]
        bot_id_index = (bot_id_index + 1) % len(bot_ids)  # 轮询到下一个bot_id
        logging.info(f"线程 {threading.current_thread().name} 使用 bot_id: {current_bot_id}")
    ...
    data = {
        "conversation_id": str(random.randint(10000000000, 99999999999)),
        "bot_id": current_bot_id,  # 使用轮询获取的bot_id
        "user": user_id,
        "query": row['info'],
        "stream": False,
        "custom_variables": {}
    ...
    if response.status_code == 200:
        ...
        # 解析coze JSON响应数据
            ...
            if 'messages' in response_json:
                ...
                if content_parts:
                    introduce = ''.join(content_parts)
                else:
                    print("未找到有效的answer内容")
            else:
    ...
    # finally:
    #     lock.release()
    return [counter,introduce, threading.current_thread().name,start_time, end_time, format_time(start_time), format_time(end_time), elapsed_time, current_bot_id]
...
def update_result_in_db(id, result):
    ...
    try:
        introduce, start_time, end_time, elapsed_time, bot_id = result[1], result[5], result[6], result[7], result[8]
        cursor.execute("""
            UPDATE zhilian_resume SET 
                ...
                process_type = '5'
            WHERE id = %s
        """, (introduce, start_time, end_time, elapsed_time, bot_id, id))
        connection.commit()
        logging.info(f"线程:{threading.current_thread().name}已更新 id：{id}...耗时：{elapsed_time}...bot_id：{bot_id}")
    except Exception as e:
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            cursor.execute("update zhilian_resume set resume_description_detail = %s, process_start_time = %s,"
                           ...
                           (row['introduce'], time_to_timestamp(row['startTime_format'],  date_str='2025-06-10'), 
                           time_to_timestamp(row['endTime_format'], date_str='2025-06-10'), row['耗时(秒)'], '2', row['id']))
            connection.commit()
