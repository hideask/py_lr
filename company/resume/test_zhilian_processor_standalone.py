...
# 模拟数据库连接模块
sys.modules['db_connection'] = type('MockModule', (), {
...
# 创建模拟的子处理器
class MockEducationExperienceProcessor:
    def process_education_experiences(self, experiences):
        # 简单的"至今"处理模拟
        for exp in experiences:
            if isinstance(exp, dict) and 'educationTimeLabel' in exp:
                time_label = exp['educationTimeLabel']
                if '至今' in time_label:
                    exp['educationTimeLabel'] = time_label.replace('至今', '2025.05')
        return experiences

class MockWorkExperienceProcessor:
    def process_work_experiences(self, experiences):
        return experiences

# 模拟子处理器模块
sys.modules['education_experience_processor'] = type('MockModule', (), {
    'EducationExperienceProcessor': MockEducationExperienceProcessor
})()

sys.modules['work_experience_processor'] = type('MockModule', (), {
    'WorkExperienceProcessor': MockWorkExperienceProcessor
})()
...
def test_deduplication_logic():
    ...
    education_data = [
        {"schoolName": "北京大学", "major": "计算机科学"},
        {"schoolName": "北京大学", "major": "计算机科学"},  # 重复
        {"schoolName": "清华大学", "major": "软件工程"}
    ]
    
    deduplicated_education = processor.deduplicate_education_experiences(education_data)
    print(f"教育经历去重: {len(education_data)} -> {len(deduplicated_education)}")
    
    # 测试工作经历去重
    work_data = [
        {"orgName": "腾讯", "jobTitle": "工程师", "timeLabel": "2022-2023"},
        {"orgName": "腾讯", "jobTitle": "工程师", "timeLabel": "2022-2023"},  # 重复
        {"orgName": "阿里", "jobTitle": "开发", "timeLabel": "2021-2022"}
    ]
    
    deduplicated_work = processor.deduplicate_work_experiences(work_data)
    print(f"工作经历去重: {len(work_data)} -> {len(deduplicated_work)}")
    
    # 测试项目经历去重
    project_data = [
        {"name": "电商平台", "timeLabel": "2023-2024"},
        {"name": "电商平台开发", "timeLabel": "2023-2024"},  # 相似项目
        {"name": "移动应用", "timeLabel": "2022-2023"}
    ]
    
    deduplicated_project = processor.deduplicate_project_experiences(project_data)
    print(f"项目经历去重: {len(project_data)} -> {len(deduplicated_project)}")

def main():
    """
    主测试函数
    """
    print("智联招聘简历数据处理器独立测试")
    print("=" * 50)
